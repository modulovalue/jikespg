...
bool is_terminal_rhs(short *rhs_start, const bool *produces_terminals, const int rule_no, struct ruletab_type *rules) {
...
void check_non_terminals(struct CLIOptions *cli_options, short *lhs_rule, short *next_rule, short *nt_list, struct ruletab_type *rules) {
  bool changed = true;
  ...
  while (changed) {
    ...
    for ALL_NON_TERMINALS3(nt) {
      int rule_no;
      for (bool end_node = (rule_no = lhs_rule[nt]) == NIL; !produces_terminals[nt] && !end_node; end_node = rule_no == lhs_rule[nt]) {
        rule_no = next_rule[rule_no];
        if (is_terminal_rhs(rhs_start, produces_terminals, rule_no, rules)) {
          changed = true;
...
void no_rules_produced(struct CLIOptions *cli_options, short *lhs_rule, short *nt_list) {
...
void compute_closure(const int lhs_symbol, short *stack, short *index_of, struct ProduceTop *topp, short *lhs_rule, short *next_rule, struct node **closure, struct ruletab_type *rules) {
  short *nont_list = Allocate_short_array(num_non_terminals);
  nont_list -= num_terminals + 1; /* Temporary direct        */
  // access set for closure.
  stack[++topp->top] = lhs_symbol;
    ...
    if (IS_A_NON_TERMINAL(symbol)) {
      if (nont_list[symbol] == OMEGA) {
        ...
        if (index_of[symbol] == OMEGA) {
          compute_closure(symbol, stack, index_of, topp, lhs_rule, next_rule, closure, rules);
        }
  ...
  if (index_of[lhs_symbol] == indx) {
    for (int symbol = stack[topp->top]; symbol != lhs_symbol; symbol = stack[--topp->top]) {
...
bool is_nullable_rhs(short *rhs_start, const int rule_no, bool *null_nt, struct ruletab_type *rules) {
...
void compute_first(const int nt, struct DetectedSetSizes *dss, short *stack, short *index_of, struct ProduceTop *topp, JBitset nt_first, short *lhs_rule, short *next_rule, bool *null_nt, struct ruletab_type *rules) {
  JBitset temp_set;
  ...
  for (bool end_node = (rule_no = lhs_rule[nt]) == NIL; !end_node; /* Iterate over all rules produced by NT */ end_node = rule_no == lhs_rule[nt]) {
    ...
    for ENTIRE_RHS3(i, rule_no) {
      ...
      if (IS_A_NON_TERMINAL(symbol)) {
        if (index_of[symbol] == OMEGA) {
          compute_first(symbol, dss, stack, index_of, topp, nt_first, lhs_rule, next_rule, null_nt, rules);
        }
  ...
  if (index_of[nt] == indx) {
    for (int symbol = stack[topp->top]; symbol != nt; symbol = stack[--topp->top]) {
...
///       That is, ROOT <= TAIL !!!
short first_map(const int root, const int tail, struct ProduceTop *topp, struct f_element_type *first_element, short *first_table) {
  ...
  first_element[topp->top].link = first_table[rhs_sym[root]];
  first_table[rhs_sym[root]] = topp->top;
  return topp->top;
}
...
void compute_follow(const int nt, struct DetectedSetSizes *dss, short *stack, short *index_of, struct ProduceTop *topp, JBitset follow, short *next_item, short *nt_items, JBitset first, struct ruletab_type *rules, struct itemtab *item_table) {
  JBitset temp_set;
  calloc0_set(temp_set, 1, dss->term_set_size);
  ...
  if (index_of[nt] == indx) {
    for (int lhs_symbol = stack[topp->top]; lhs_symbol != nt; lhs_symbol = stack[--topp->top]) {
...
struct DetectedSetSizes mkbasic(struct CLIOptions *cli_options, JBitset nt_first, bool * *rmpself, JBitset* first, struct FirstDeps* fd, struct ruletab_type *rules) {
  ...
  short *next_rule = Allocate_short_array(NEXT_RULE_SIZE());
  short *first_item_of = Allocate_short_array(NEXT_RULE_SIZE());
  short *stack = Allocate_short_array(num_non_terminals + 1);
  short *index_of = Allocate_short_array(num_non_terminals);
  ...
  short *nt_items = Allocate_short_array(num_non_terminals);
  ...
  for ALL_NON_TERMINALS3(nt) {
    if (index_of[nt] == OMEGA) {
      compute_closure(nt, stack, index_of, &topp, lhs_rule, next_rule, fd->closure, rules);
    }
  }
    ...
    while (changed) {
      ...
      for ALL_NON_TERMINALS3(nt) {
        ...
        for (bool end_node = (rule_no = lhs_rule[nt]) == NIL; !null_nt[nt] && !end_node; end_node = rule_no == lhs_rule[nt]) {
          rule_no = next_rule[rule_no];
          if (is_nullable_rhs(rhs_start, rule_no, null_nt, rules)) {
            changed = true;
            null_nt[nt] = true;
          }
        }
  ...
  for ALL_NON_TERMINALS3(nt) {
    if (index_of[nt] == OMEGA) {
      compute_first(nt, &dss, stack, index_of, &topp, nt_first, lhs_rule, next_rule, null_nt, rules);
    }
  }
  ...
  check_non_terminals(cli_options, lhs_rule, next_rule, nt_list, rules);
  // Construct the ITEM_TABLE, FIRST_ITEM_OF, and NT_ITEMS maps.
  short *first_table = Allocate_short_array(num_symbols + 1);
    ...
    for ENTIRE_RHS3(i, rule_no) {
      ...
      if (cli_options->lalr_level > 1 ||
        ...
        if (i == k) {
          ...
          item_table[item_no].suffix_index = first_map(i + 1, k, &topp, first_element, first_table);
        }
      } else {
        item_table[item_no].suffix_index = NIL;
      }
  ...
  if (error_maps_bit || next_rule[lhs_rule[accept_image]] != lhs_rule[accept_image]) {
    ...
    if (next_rule[rule_no] != rule_no) {
      ...
      topp.top = item_table[first_item_of[rule_no]].suffix_index;
      for (int i = topp.top + 1; i <= num_first_sets; i++) {
        rule_no = next_rule[rule_no];
        item_no = first_item_of[rule_no];
