...
void check_non_terminals(struct CLIOptions* cli_options, short *lhs_rule, short *next_rule, short *nt_list) {
  bool changed = true;
...
void compute_closure(const int lhs_symbol, short *stack, short* index_of, struct ProduceTop* topp, short *lhs_rule, short *next_rule) {
    ...
    if (IS_A_NON_TERMINAL(symbol)) {
      if (nont_list[symbol] == OMEGA) {
        ...
        if (index_of[symbol] == OMEGA) {
          compute_closure(symbol, stack, index_of, topp, lhs_rule, next_rule);
        }
...
void compute_first(const int nt, struct DetectedSetSizes* dss, short *stack, short* index_of, struct ProduceTop* topp, JBitset nt_first, short *lhs_rule, short *next_rule) {
  JBitset temp_set;
  ...
  for (bool end_node = (rule_no = lhs_rule[nt]) == NIL; !end_node; /* Iterate over all rules produced by NT */ end_node = rule_no == lhs_rule[nt]) {
    ...
    for ENTIRE_RHS3(i, rule_no) {
      ...
      if (IS_A_NON_TERMINAL(symbol)) {
        if (index_of[symbol] == OMEGA) {
          compute_first(symbol, dss, stack, index_of, topp, nt_first, lhs_rule, next_rule);
        }
...
void compute_follow(const int nt, struct DetectedSetSizes* dss, short *stack, short* index_of, struct ProduceTop* topp, JBitset follow, short *next_item, short *nt_items) {
  JBitset temp_set;
  calloc0_set(temp_set, 1, dss->term_set_size);
  ...
  for (int item_no = nt_items[nt]; item_no != NIL; item_no = next_item[item_no]) {
    ...
    if (IS_ELEMENT(temp_set, empty)) {
      ...
      if (index_of[lhs_symbol] == OMEGA) {
        compute_follow(lhs_symbol, dss, stack, index_of, topp, follow, next_item, nt_items);
      }
...
struct DetectedSetSizes mkbasic(struct CLIOptions* cli_options, JBitset nt_first) {
  ...
  for ALL_NON_TERMINALS3(nt) {
    if (index_of[nt] == OMEGA) {
      compute_closure(nt, stack, index_of, &topp, lhs_rule, next_rule);
    }
  }
  ...
  for ALL_NON_TERMINALS3(nt) {
    if (index_of[nt] == OMEGA) {
      compute_first(nt, &dss, stack, index_of, &topp, nt_first, lhs_rule, next_rule);
    }
  }
  ...
  check_non_terminals(cli_options, lhs_rule, next_rule, nt_list);
  ...
  if (error_maps_bit || next_rule[lhs_rule[accept_image]] != lhs_rule[accept_image]) {
    ...
    for ALL_NON_TERMINALS3(nt) {
      if (index_of[nt] == OMEGA) {
        // not yet computed ?
        compute_follow(nt, &dss, stack, index_of, &topp, follow, next_item, nt_items);
      }
    }
