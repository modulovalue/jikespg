...
void compute_follow(const int nt, struct DetectedSetSizes *dss, short *stack, short *index_of, struct ProduceTop *topp, JBitset follow, short *next_item, short *nt_items, JBitset first) {
  JBitset temp_set;
  calloc0_set(temp_set, 1, dss->term_set_size);
  ...
  for (int item_no = nt_items[nt]; item_no != NIL; item_no = next_item[item_no]) {
    ...
    if (IS_ELEMENT(temp_set, empty)) {
      ...
      if (index_of[lhs_symbol] == OMEGA) {
        compute_follow(lhs_symbol, dss, stack, index_of, topp, follow, next_item, nt_items, first);
      }
